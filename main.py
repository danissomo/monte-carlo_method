...
def write_results_to_file(filename, time_stamp=[],  q=[], qd=[], qdd=[], grav_torques=[], inertia_torques=[], coriolis=[]):
    ...
    for i in range(size):
        ...
        for j in range(n):
            file.write("{:.9f}".format(q[i][j]) + ",")
        file.write(",")
        for j in range(n):
            file.write("{:.9f}".format(qd[i][j]) + ",")
        file.write(",")
        for j in range(n):
            file.write("{:.9f}".format(qdd[i][j]) + ",")
        file.write(",")
        for j in range(n):
            file.write("{:.9f}".format(grav_torques[i][j]) + ",")
        file.write(",")
        for j in range(n):
            file.write("{:.9f}".format(inertia_torques[i][j]) + ",")
        file.write(",")
        for j in range(n):
            file.write("{:.9f}".format(coriolis[i][j]) + ",")
        file.write(f"\n")
    file.close()
...
def main():
    ...
    for i in range(1, test_step_count):
        ...
        if i > 140 and i <160 : print(qdd[i])
